...
## `coupled-feedback/` — the missing coupling substrate, proven small and useful
**Verdict: the one layer Morphogen never built is ~15 lines and it matters.** An engine idle
governor co-advances two rigorous domains (`integrators` mechanics ↔ `controls` PID) with real
per-timestep feedback. Governor engaged holds 800 RPM under a load lurch (RMS error 13.7);
a *fairly-tuned* fixed-throttle open loop droops to ~540 RPM (RMS error 136.3) — feedback
coupling wins **~10×**. Sonified to WAV + plotted (`engine_governor.png`). This is what the
one-shot composer fundamentally cannot do.
Run: `python coupled-feedback/engine_governor.py`.
